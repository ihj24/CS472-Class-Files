...
void get_current_ntp_time(ntp_timestamp_t *ntp_ts)
{
    ...
    unsigned long long fraction_calc;

    gettimeofday(&now, NULL);
    ntp_seconds = now.tv_sec + NTP_EPOCH_OFFSET;
    ntp_ts->seconds = htonl(ntp_seconds);

    fraction_calc = (now.tv_usec * NTP_FRACTION_SCALE) / USEC_INCREMENTS;
    ntp_ts->fraction = htonl((uint32_t)fraction_calc);
...
double ntp_time_to_double(const ntp_timestamp_t *timestamp)
{
    ...
    seconds = (double)ntohl(timestamp->seconds);
...
void print_ntp_time(const ntp_timestamp_t *ts, const char *label, int local)
{
    ...
    ntp_time_to_string(ts, buffer, TIME_BUFF_SIZE, local);
    printf("%s: %s (%s)\n", label, buffer, local ? "Local Time" : "UTC");
}
...
int decode_reference_id(uint8_t stratum, uint32_t ref_id, char *buff, int buff_sz)
{
    ...
    if (stratum >= 2)
    {
        ...
        if (inet_ntop(AF_INET, &net_ref_id, buff, buff_sz) == NULL)
        {
            strcpy(buff, "INVALID");
            return RC_OK;
        }
...
void print_ntp_results(const ntp_result_t *result)
{
    ...
    printf("=== NTP Time Synchronization Results ===\n");
