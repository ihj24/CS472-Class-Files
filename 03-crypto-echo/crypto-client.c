...
int client_loop(int sockfd)
{
    ...
    while (1)
    {
        ...
        // Get user command
        if (get_command(input_buffer, MAX_MSG_DATA_SIZE, &command) != CMD_EXECUTE)
        {
            continue;
        }

        // Build and send PDU
        int pdu_size = build_packet(&command, (crypto_msg_t *)send_buffer, session_key);
        send(sockfd, send_buffer, pdu_size, 0);
        ...
        // Receive response
        int bytes_received = recv(sockfd, recv_buffer, MAX_MSG_SIZE, 0);
        if (bytes_received <= 0)
        {
        ...
        // Extract key if key exchange response
...
int build_packet(const msg_cmd_t *cmd, crypto_msg_t *pdu, crypto_key_t key)
{
    ...
    {
        // Plain text message
        int len = strlen(cmd->cmd_line);
        memcpy(pdu->payload, cmd->cmd_line, len);
        pdu->header.payload_len = len;
    ...
    else if (cmd->cmd_id == MSG_ENCRYPTED_DATA)
    {
        ...
        {
            printf("[ERROR] No session key established.\n");
            return 0;
        }
        int encrypted_len = encrypt_string(key, pdu->payload,
