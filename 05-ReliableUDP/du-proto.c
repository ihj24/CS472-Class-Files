...
#define DP_MT_FRAGMENTACK (DP_MT_FRAGMENT | DP_MT_ACK)
static int dpsendfragment(dp_connp dp, void *sbuff, int sbuff_sz);
...
static int dprecvdgram(dp_connp dp, void *buff, int buff_sz)
{
    ...
    case DP_MT_SND:
        ...
        break;
    case DP_MT_FRAGMENT:
        outPdu.mtype = DP_MT_FRAGMENTACK;
        actSndSz = dpsendraw(dp, &outPdu, sizeof(dp_pdu));
        if (actSndSz != sizeof(dp_pdu))
            return DP_ERROR_PROTOCOL;
        break;
...
static int dpsendfragment(dp_connp dp, void *sbuff, int sbuff_sz)
{
    ...
    {
        perror("dpsendfragment: dp connection not setup properly");
    ...
    int totalSendSz = sbuff_sz + sizeof(dp_pdu);
    bytesOut = dpsendraw(dp, _dpBuffer, totalSendSz);

    dp->seqNum += sbuff_sz;

    /* Wait for FRAGMENTACK before sending the next chunk */
    dp_pdu inPdu = {0};
    int bytesIn = dprecvraw(dp, &inPdu, sizeof(dp_pdu));
    if ((bytesIn < (int)sizeof(dp_pdu)) || (inPdu.mtype != DP_MT_FRAGMENTACK))
        printf("Expected FRAGMENT/ACK but got mtype %d\n", inPdu.mtype);
...
int dpsend(dp_connp dp, void *sbuff, int sbuff_sz)
{
    ...
    printf("dpsend: total size = %d bytes\n", sbuff_sz);

    while (total_sent < sbuff_sz)
    {

        // remaining bytes
        int chunk = sbuff_sz - total_sent;

        if (chunk > DP_MAX_BUFF_SZ)
            chunk = DP_MAX_BUFF_SZ;

        int is_last = ((total_sent + chunk) >= sbuff_sz);

        printf("dpsend: sending chunk %d-%d (%s)\n",
               total_sent,
               total_sent + chunk,
               is_last ? "FINAL" : "FRAGMENT");

        int rc;
        if (is_last)
            rc = dpsenddgram(dp, src + total_sent, chunk);
        else
            rc = dpsendfragment(dp, src + total_sent, chunk);
    ...
    printf("dpsend: done, total sent = %d\n", total_sent);
    return total_sent;
}
